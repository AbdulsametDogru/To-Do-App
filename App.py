...
# --- 🔥 ULTRA PREMIUM CAFCAFLI & RENKLİ TÜM BİLEŞENLER CSS MİMARİSİ ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap');
    * { font-family: 'Plus Jakarta Sans', sans-serif !important; }
    
    /* 🌌 ANA ARKA PLAN (HIGH-TECH RADIAL) */
    .stApp {
        background: 
    ...
    /* 🎨 RENKLİ KONTROL MERKEZİ (SIDEBAR) TASARIMI */
    ...
    /* 🎯 RENKLİ KARTLAR VE GÖMÜLÜ BUTONLAR */
    .custom-task-card {
    ...
    /* 🎨 GÜNCELLEME EKRANI (DIALOG/MODAL) RENKLERİ */
    div[role="dialog"] {
...
# --- 📝 GÖREV DÜZENLEME MODALI (RENKLİ SÜRÜM) ---
...
st.markdown(f"""
    <div style='margin-bottom: 25px;'>
        ...
        <p style='color: #64748b; margin: 0; font-size: 13px;'>Tüm kontrol ve düzenleme panelleri renkli teknik arayüze kavuşturuldu.</p>
    </div>
""", unsafe_allow_html=True)
...
# --- ⚙️ KONTROL MERKEZİ (SIDEBAR - RENKLİ) ---
...
for anahtar, (st_sutun, baslik, renk) in ayarlar.items():
    ...
    with st_sutun:
        ...
        for g in filtre_gorevler:
            ...
            k_metin = f"Gecikti ({abs(k_gun)}g)" if k_gun < 0 else ("Son Gün" if k_gun == 0 else f"{k_gun} gün")
            st.markdown(f"<div class='custom-task-card card-glow-{g.zorluk}'><div class='task-title-area'><span>{g.ad}</span><span class='badge-white'>{g.zorluk}</span></div><div class='task-time-area'><span>📅 {g.son_tarih}</span><span style='font-weight:700; color:{'#ff8585' if k_gun<=0 else '#ffffff'}'>{k_metin}</span></div></div>", unsafe_allow_html=True)
